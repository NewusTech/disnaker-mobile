...
import { useCreateExperienceHistory } from "@/services/user";
...
export default function index() {
  ...
  const createExperienceHistory = useCreateExperienceHistory();

  const handleLoginMutation = handleSubmit(async (data) => {
    const htmlValue = await editor.getHTML();
    createExperienceHistory.mutate(
      {
        ...data,
      ...
      },
      {
        onSuccess: async (response) => {
          Toast.show({
            type: "success",
            text1: "Add Pengalaman Kerja History Berhasil!",
        ...
        onError: (reponse) => {
          ...
          Toast.show({
            type: "error",
            text1: "Add Pengalaman Kerja History Gagal!",
  ...
  return (
    <View backgroundColor="white" style={{ flex: 1 }}>
      ...
      >
        ...
        <Button
          style={{ marginTop: 20 }}
          disabled={!formState.isValid || createExperienceHistory.isPending}
          onPress={handleLoginMutation}
        >
          {createExperienceHistory.isPending ? (
            <Loader color="white" />
          ) : (
            "Simpan"
          )}
