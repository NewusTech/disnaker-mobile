...
export default function index() {
  ...
  const sertificate = useGetSertificateById(params.id);
  ...
  const updateSertificate = useUpdateSetificate();
  const deleteSertificate = useDeleteSetificate();
  ...
  const handleDeleteMutation = () => {
    deleteSertificate.mutate(params.id, {
      onSuccess: async (response) => {
        Toast.show({
          type: "success",
          text1: "Delete Sertificate Berhasil!",
          text2: response.message,
        });
        router.dismiss();
      },
      onError: (reponse) => {
        ...
        Toast.show({
          type: "error",
          text1: "Delete Sertificate Gagal!",
          text2: reponse.response?.data.message,
        });
      },
    });
  ...
  useEffect(() => {
    const _sertificate = sertificate.data?.data;
    if (_sertificate) {
      setValue("desc", _sertificate.desc);
      setValue("expiredDate", new Date(_sertificate.expiredDate || 0));
      setValue("isNonExpire", _sertificate.isNonExpire === "true");
      setValue("organization", _sertificate.organization);
      setValue("title", _sertificate.name);
    }
  }, [sertificate.data?.data]);
  ...
  return (
    <View backgroundColor="white" style={{ flex: 1 }}>
        ...
        <View style={{ marginTop: 20, gap: 20 }}>
          ...
          <View style={{ marginTop: 10 }}>
            <UploadFile
              ...
              fileUrl={sertificate.data?.data.file || ""}
            />
          </View>
        </View>
        <Button
          style={{ marginTop: 20 }}
          disabled={!formState.isValid || updateSertificate.isPending}
          onPress={handleLoginMutation}
        >
          {updateSertificate.isPending ? <Loader color="white" /> : "Update"}
        </Button>
        <Button
          style={{ marginTop: 20 }}
          color="error-60"
      ...
      <ModalAction
        ...
        isLoading={deleteSertificate.isPending}
      />
    </View>
  );
}
